...
// returns last pair to be connected
private func buildCircuit(_ pairs: [(Coord, Coord, Double)]) -> (Coord, Coord, Double)? {
    ...
    for i in 0..<pairs.count {
        ...
        if circuits.count == 1 && circuits[0].count == numJunctions {
            return p
        }
    }
...
private func productOfLastxs(_ lastConnection: (Coord, Coord, Double)) -> Int {
...
func day8part2() {
    ...
    if let last = buildCircuit(pairs) {
        print(productOfLastxs(last))
    } else {
        print("not getting to a single circuit")
    }
}
